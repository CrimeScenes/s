...
local function isWithinBoxFOV(position)
    ...
    -- Create box FOV
    local fovBox = Rect.new(screenCenter.X - fovWidth / 2, screenCenter.Y - fovHeight / 2, fovWidth, fovHeight)
    return fovBox:Contains(Vector2.new(screenPos.X, screenPos.Y))
end
...
local function getPredictedPosition(character)
    ...
    if primaryPart then
        ...
        local timeToPredict = 0.15 -- Adjust as necessary for different game speeds

        -- Predict future position based on velocity and current position
            ...
            if humanoid and humanoid.Health > 0 then
                ...
                for _, partName in ipairs(getgenv().Forbidden.TriggerBot.Settings.Parts) do
                    ...
                    if part then
                        ...
                        -- Check if within box FOV and closer than the previous closest
